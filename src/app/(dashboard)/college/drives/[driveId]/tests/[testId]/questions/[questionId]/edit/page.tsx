...
export default function EditQuestionPage() {
  ...
  useEffect(() => {
    if (questionsQuery.error) {
      toast.error(questionsQuery.error.message);
      router.push(`/college/drives/${params.driveId}/tests/${params.testId}`);
    }
  }, [questionsQuery.error, params.driveId, params.testId, router]);

  // Populate form state when data loads
  useEffect(() => {
    if (!questionsQuery.data || isInitialized) return;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const q = questionsQuery.data.find((q: any) => q.id === params.questionId) as any;
    if (!q) {
    ...
    setOptions(
      ((q.options ?? []) as { id: string; text: string }[]).map((o: { id: string; text: string }) => ({
        id: o.id,
        text: o.text,
      }))
    );
    setCorrectOptionIds((q.correctOptionIds ?? []) as string[]);
    setIsInitialized(true);
  }, [questionsQuery.data, isInitialized, params.questionId, params.driveId, params.testId, router]);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
...

    {
      ...
      updateMutation.mutate({
        ...
        questionType: questionType as "SINGLE_SELECT" | "MULTI_SELECT" | "CODING",
